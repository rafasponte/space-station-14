...
<<<<<<< HEAD
>>>>>>> 878cbb9aeb (Added command button)
=======
        // Main UI dialog
>>>>>>> 13c4c59313 (Keys are being binded)
...
>>>>>>> d640f41763 (Command dialog box is centered and has background)
=======
                HorizontalAlignment = HAlignment.Left,
                // StyleClasses = { StyleBase.ButtonCaution } // Optional: adds visual importance
>>>>>>> 5dadcd078c (feat: Created custom keybind dialog for a new one.)
=======
                HorizontalAlignment = HAlignment.Left
>>>>>>> 13c4c59313 (Keys are being binded)
            };
...
=======
>>>>>>> 1079ae776c (feat: Now adds the button everytime you bind the key)
...
>>>>>>> 9ebebb4104 (feat: #36211 GUI for creating command binds in the controls tab)
=======
            _addCommandDialog = new BoxContainer
=======
            var contentBox = new BoxContainer
>>>>>>> d640f41763 (Command dialog box is centered and has background)
            {
=======
>>>>>>> 019dad642f (fix: Fixed issues with the line edit button to be bigger, changes some UI measurements and fixed the issue with clicking through the dialog)
                Orientation = LayoutOrientation.Vertical,
                Margin = new Thickness(8),
                Children =
                {
